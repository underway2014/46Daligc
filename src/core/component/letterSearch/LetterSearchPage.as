package core.component.letterSearch
{
	...
	import core.config.CommonData;
	import core.config.LetterSearchData;
	...
	public class LetterSearchPage extends Sprite
	{
		...
		private function setDMBtnMouseEnable():void
		{
			if(dmBtn)
			{
				dmBtn.mouseChildren = dmBtn.mouseEnabled = true;
			}
		}
...
//			contentMask.x = data.sid == 0 ? 0 : (int(data.sid/ITEM_NUM_PER_PAGE+1))*PAGE_WIDTH;;
		...
		private function stepLoader(event:TimerEvent):void
		{
			...
			for (var i:int=currentIndex; i<endIndex; i++)
			{
				...
				if (letterCurData.type == "book")	//DM单
				{
					...
					btn.data = [letterCurData.url,letterCurData.name];
					btn.addEventListener(MouseEvent.CLICK,dmClickHandler);
					contentPannel.addChild(btn);
					btnArr.push(btn);
				}else if(letterCurData.type == "mobile")//mobile
				{
					var discmSrcArr:Array = new Array(letterCurData.cover, commonData.publicUrl+commonData.disc, 
						commonData.publicUrl+commonData.discBG);
					var discm:DiscCover = new DiscCover(discmSrcArr);
					contentLayout.add(discm);
					discm.data = ["yunnan/" + letterCurData.url,letterCurData.name,"mobile"];
					discm.addEventListener(MouseEvent.CLICK,contentClickHandler);
					contentPannel.addChild(discm);
					btnArr.push(discm);
				}
				else	//全攻略
				{
					...
					disc.data = [letterCurData.url,letterCurData.name];
		...
		private function initContent():void
		{
			...
			trace("loaderScenicList.getLength() = " + loaderScenicList.getLength());
...
//			contentPannel.addChildAt(bgs,0);
		...
		private function initMask():void
		{
			...
			contentMask.graphics.beginFill(0xFFFFFF,.3);
			contentMask.graphics.drawRect(45,0,995,615);
		...
		private function dmClickHandler(event:Event):void
		{
			...
			currentUrl = dmBtn.data[0];
			Administrator.sendData(dmBtn.data[1]);
			if(!currentUrl)	return;
			currentType = "DM";
//			dmBtn.mouseChildren = dmBtn.mouseEnabled = false;
			maskShape.visible = true;
...
//			loader._loader.focusRect = null;
		...
		private function contentClickHandler(event:Event):void
		{
			...
			currentUrl = btn.data[0];
			Administrator.sendData(btn.data[1]);
			if(!currentUrl)	return;
			if(arr.length > 2)
			{
				currentType = "MB";
...
//			trace("loaderScenicList.getLength() = ",loaderScenicList.getLength());
			...
			if(curSid<0)
			{
				if(loaderScenicList.getLength() % ITEM_NUM_PER_PAGE == 0)
				{
					curSid =((loaderScenicList.getLength()/ITEM_NUM_PER_PAGE) - 1)*ITEM_NUM_PER_PAGE;
				}else{
		...
		private function addRemoveListener():void
		{
			...
			var sjarr:Array = ["algd2Event","bhsdEvent","bmcEvent","bsgz2Event","bzl2Event","cjdwhsq2Event","cjdywEvent","cmgdEvent","csehEvent","cssst2Event",
				"cyyhgEvent","dc2Event","dchtdEvent","dggy2Event","dghlhEvent","dkzgcEvent","dlgc2Event","dsbEvent","dsgEvent","dyhEvent",
				"dzy2Event","fxh2Event","gdgz2Event","gnbbEvent","hjgz2Event","hltgy2Event","hsgz2Event","htx2Event","hzgc2Event","jd2Event",
				"jlpb2Event","jpfsl2Event","jsgc2Event","jx2Event","jzsEvent","kmsby2Event","kmwm2Event","lbgxsxEvent","lgh2Event","ljgc2Event",
				"ljljs2Event","ljsqtqbwg2Event","ljwbs2Event","llcssl2Event","lqslgyEvent","lshEvent","mbnxzqyEvent","mjbhgxbzy2Event","mldfsEvent","mlplsEvent",
				"mlrdylEvent","mlxs2Event","mtgyEvent","mwblchEvent","ndgc2Event","ndxfss2Event","nggzEvent","nygzEvent","nzfqd2Event","pdc2Event",
				"pls2Event","pzh2Event","rdhhyEvent","rdzwy2Event","rljdyj2Event","sbs2Event","sfxbdxgEvent","shgz2Event","sjklgEvent","slgz2Event",
				"sms2Event","sxgz2Event","szls2Event","tchs2Event","tcrh2Event","tlbbysc2Event","wbs2Event","wdszsEvent","wsgc2Event","wtsEvent",
				"xslswhgy2Event","xsslgy2Event","xzgz2Event","ydsx2Event","ylhEvent","ylxs2Event","ymtl2Event","ynmzbwg2Event","ynmzc2Event","ynsbwg2Event",
				"ynsl2Event","yrgzEvent","ysslgy2Event","ysz2Event","yts2Event","yxg2Event","yyhntt2Event","yzdEvent","zjhyEvent","zjyEvent"];
